...
fit_gauss_indep <- function(data, mesh, group, repl, n, n_opt) {
  # Fit Gaussian independent model
  result <- ngme(
    Y ~ 0 + f(~ Long + Lat,
      mesh = mesh,
      model = "bv_matern_normal",
      sub_models = list(Psal = "matern", Temp = "matern"),
      noise = list(Psal = noise_normal(), Temp = noise_normal()),
...
fit_gauss_cor <- function(data, mesh, group, repl, n, n_opt) {
  # Fit Gaussian independent model
  result <- ngme(
    Y ~ 0 + f(~ Long + Lat,
      mesh = mesh,
...
fit_model_new <- function(iGrid, modelType, presLevel) {
  ...
  base_dir <- "Results" # Since the script is launched from 'ibex_Scripts', the base directory is one level up.
  ...
  if (!file.exists(save_filename)) {
    ...
    # change the model as to account for new updates in ngme2 code
    if (modelType == "gauss_indep") {
      result_new <- fit_gauss_indep(data$data, data$mesh, data$group, data$repl, data$n, n_opt)
    } else if (modelType == "gauss_cor") {
      result_new <- fit_gauss_cor(data$data, data$mesh, data$group, data$repl, data$n, n_opt)
    }
    end.time <- Sys.time()
